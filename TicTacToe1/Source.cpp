...
void Move(char field[], const int n, char player)
{
	...
	do
	{
		...
		else if (field[key - 49] != 0)cout << "Error:êëåòêà óæå çàíÿòà" << endl;
...
void Check(char field[], const int n, char player)
{
	...
	if (field[2] == field[4] && field[4] == field[8] && field[4] != 0)game_over = true;

	if (field[0] == field[1] && field[1] == field[2] && field[0] != 0)game_over = true;
	if (field[3] == field[1] && field[1] == field[2] && field[3] != 0)game_over = true;
